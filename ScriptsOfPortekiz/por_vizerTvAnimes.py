...
class VizerTvSeries:
    ...
    def seriePages(self, baseUrl):
        ...
        while (i <= 30):
            url = baseUrl + str(
                i) + "&saga=0&categoryFilterYearMin=1980&categoryFilterYearMax=2022&categoryFilterOrderBy=vzViews&categoryFilterOrderWay=desc"
            serieIdList = []
    ...
    def getGenreList(self, imdbId):
        ...
        try:
            selector = self.getImdbUrl(imdbId=imdbId)
            genre = selector.xpath("//li[@class='ipc-inline-list__item ipc-chip__text']//text()").getall()
            for gen in genre:
...
if __name__ == '__main__':
     ...
     VizerTvSeries().seriePages(
          baseUrl="https://vizer.tv/includes/ajax/ajaxPagination.php?categoriesListSeries=all&anime=1&search=&page=")
